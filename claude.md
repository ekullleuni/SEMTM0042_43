...
## Notes on behaviour
 ...
 - You MUST log every design update in `docs/Design_log.md`. A "design update" includes: measured physical constants (map dimensions, gradient centres, wheel geometry), calibration outcomes, chosen parameter values with their rationale, architectural decisions, and rejected alternatives with the reason. Each entry must be dated (absolute, `YYYY-MM-DD`), placed at the top of the file (reverse-chronological), and must record **what** was decided/measured, **why**, and **how it was derived or verified** so a future session can re-verify. Keep the log distinct from `Project_log.md` (state/bugs/history) and `Development_plan.md` (roadmap) — `Design_log.md` is for durable design facts. If the file does not exist, create it.